...
public interface DynDiscAssignRepository extends JpaRepository<DynDiscAssign, Long> {
	...
	@Query(nativeQuery = true,value = """
		INSERT INTO dyn_disc_base_temp
			WITH
			  /*--------------------------------------------
			...
			FROM va_offer_alo_full_view va
						""")
			int persistAssignedDiscountsToTempTable ( Integer requestId, @Param("targetDate") LocalDateTime targetDate);
	...
	@Query(nativeQuery = true,value = """
			...
			FROM va_offer_alo_full_view va
						""")
			List<DynDiscAssignDTO> fetchAllAssignedDiscounts ( Integer requestId, @Param("targetDate") LocalDateTime targetDate);
	
	    // Truncate the temp table
	    @Modifying
	    @Query(value = "TRUNCATE TABLE dyn_disc_base_temp", nativeQuery = true)
	    ...
	    @Query(value = """
	    	    SELECT *
	    	    FROM dyn_disc_base_temp
	    	    WHERE co_id IN :coIds
	    	      AND request_id = :requestId
	    	""", nativeQuery = true)
		List<DynDiscAssignDTO> fetchAssignedDiscounts(List<Integer> coIds, Integer requestId);
	    ...
	    @Query(value = """
	    	...
	    	""", nativeQuery = true)
		List<DynDiscAssignDTO> fetchAllDiscountsFromTempTable(Integer requestId);
